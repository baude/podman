GO ?= go
EPOCH_TEST_COMMIT ?= f258e43c7c4685102fbbf52303348f434ca8cb3e
HEAD ?= HEAD
CHANGELOG_BASE ?= HEAD~
...
ifneq (,$(findstring varlink,$(BUILDTAGS)))
	PODMAN_VARLINK_DEPENDENCIES = cmd/podman/varlink/iocontainerspodman.go
endif
...
clean:
	rm -rf \
		...
		cmd/podman/varlink/iocontainerspodman.go \
		$(MANPAGES) ||:
ifdef HAS_PYTHON3
...
install.systemd:
	install ${SELINUXOPT} -m 644 -D contrib/varlink/io.containers.podman.socket ${SYSTEMDDIR}/io.containers.podman.socket
	install ${SELINUXOPT} -m 644 -D contrib/varlink/io.containers.podman.service ${SYSTEMDDIR}/io.containers.podman.service
...
varlink_generate: .gopathok cmd/podman/varlink/iocontainerspodman.go
...
cmd/podman/varlink/iocontainerspodman.go: cmd/podman/varlink/io.containers.podman.varlink
	$(GO) generate ./cmd/podman/varlink/...

API.md: cmd/podman/varlink/io.containers.podman.varlink
	$(GO) generate ./docs/...
