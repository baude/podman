...
func (q *QEMUStubber) CreateVM(opts define.CreateVMOpts, mc *vmconfigs.MachineConfig) error {
	...
	qemuConfig.QEMUPidPath = qemuPidPath

	qemuConfig.Command = command.NewQemuBuilder(qemuBinary, q.addArchOptions(nil))
	qemuConfig.Command.SetBootableImage(mc.ImagePath.GetPath())
	qemuConfig.Command.SetMemory(mc.Resources.Memory)
	qemuConfig.Command.SetCPUs(mc.Resources.CPUs)
	qemuConfig.Command.SetIgnitionFile(*ignitionFile)
	qemuConfig.Command.SetQmpMonitor(monitor)
	qemuConfig.Command.SetNetwork()
	qemuConfig.Command.SetSerialPort(*readySocket, *qemuPidPath, mc.Name)
	...
	for _, mount := range mc.Mounts {
		...
		qemuConfig.Command.SetVirtfsMount(mount.Source, mount.Tag, securityModel, mount.ReadOnly)
	}
...
func (q *QEMUStubber) StartVM(mc *vmconfigs.MachineConfig) (func() error, func() error, error) {
	...
	attr.Files = files
	cmdLine := mc.QEMUHypervisor.Command
