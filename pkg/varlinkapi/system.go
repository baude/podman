...
// GetVersion ...
func (i *LibpodAPI) GetVersion(call iocontainerspodman.VarlinkCall) error {
	versionInfo, err := libpod.GetVersion()
	if err != nil {
		return err
	}

	return call.ReplyGetVersion(iocontainerspodman.Version{
...
func (i *LibpodAPI) Ping(call iocontainerspodman.VarlinkCall) error {
	return call.ReplyPing(iocontainerspodman.StringResponse{
		Message: "OK",
	})
}

// GetInfo returns details about the podman host and its stores
func (i *LibpodAPI) GetInfo(call iocontainerspodman.VarlinkCall) error {
	podmanInfo := iocontainerspodman.PodmanInfo{}
	...
	host := info[0].Data
	infoHost := iocontainerspodman.InfoHost{
	...
	store := info[1].Data
	pmaninfo := iocontainerspodman.InfoPodmanBinary{
	...
	graphStatus := iocontainerspodman.InfoGraphStatus{
	...
	infoStore := iocontainerspodman.InfoStore{
