...
func cleanup(d dnsMasqFile) error {
	_ = d.stop()
	return os.RemoveAll(filepath.Dir(d.PidFile))
}

var _ = Describe("dns masq tests", func() {
  ...
  "prevResult": {
	...
	It("dns masq add", func() {
		...
		err := originalNS.Do(func(ns.NetNS) error {
			...
			files, err := ioutil.ReadDir("/run/containers/cni/dnsmasq/test")
			Expect(err).To(BeNil())
