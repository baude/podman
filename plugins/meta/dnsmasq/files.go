...
// maskLock embeds the CNI disk lock so we can hang methods from it
type maskLock struct {
	lock *disk.FileLock
}

// release unlocks and closes the disk lock
func (m *maskLock) release() error {
...
// getLock returns a maskLock. the lock should be that of the configuration
// directory for the domain.
func getLock(path string) (*maskLock, error) {
...
func generateDNSMasqConfig(config dnsMasqFile) ([]byte, error) {
