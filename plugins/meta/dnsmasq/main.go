...
func cmdAdd(args *skel.CmdArgs) error {
	...
	masqConf, err := newDNSMasqFile(netConf.DomainName, result.Interfaces[0].Name, netConf.Name)
	if err != nil {
		return err
	}
	domainBaseDir := filepath.Dir(masqConf.PidFile)
	...
	defer func() {
		if err := lock.release(); err != nil {
			logrus.Errorf("unable to release lock for '%s': %q", masqConf.AddOnHostsFile, err)
		}
	}()
	if err := checkForDNSMasqConfFile(masqConf); err != nil {
		return err
	}
	if err := appendToFile(masqConf.AddOnHostsFile, podname, ips); err != nil {
		return err
	}
	// Now we need to HUP
	if err := masqConf.hup(); err != nil {
		return err
	}

	nameservers, err := getInterfaceAddresses(masqConf)
	if err != nil {
		return err
	}
...
func main() {
	skel.PluginMain(cmdAdd, cmdCheck, cmdDel, version.All, bv.BuildString("dnsmasq"))
}
...
func cmdCheck(args *skel.CmdArgs) error {
	...
	pid, err := masqConf.getPidProcess()
	if err != nil {
		return err
	}
	...
	files, err := ioutil.ReadDir(dnsMasqConfPath)
	if err != nil {
		return err
	}
	for _, f := range files {
...
func parseConfig(stdin []byte, args string) (*DNSMasqConf, *current.Result, string, error) {
	conf := DNSMasqConf{}
