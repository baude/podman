...
// newDNSMasqFile creates a new instance of a dnsMasqFile
func newDNSMasqFile(domainName, networkInterface, networkName string) (dnsMasqFile, error){
	dnsMasqBinary, err := exec.LookPath("dnsmasq")
	if err != nil {
		return dnsMasqFile{}, errors.New("the dnsmasq cni plugin requires the dnsmasq binary be in PATH")
	}
	masqConf := dnsMasqFile{
...
func (d dnsMasqFile) start() error {
	args := []string{
		"-u",
		"root",
...
// form of an int
func (d dnsMasqFile) getPidProcess() (*os.Process, error) {
...
func makePath(networkName, fileName string) string {
	...
	return filepath.Join(dnsMasqConfPath, networkName, fileName)
}
